...
export const paymentService = {
    ...
    async addTransaction(data: {
        ...
        return await prisma.$transaction(async (tx) => {
            ...
            const ledgerType =
                payment.category === PaymentCategory.RECEIVABLE
                    ? LedgerType.CREDIT
                    : LedgerType.DEBIT;
            ...
            if (ledgerType === LedgerType.CREDIT) {
                ...
                partyNewBalance = partyNewBalance.plus(newAmount);
            }
    ...
    async getDashboardStats(orgId: string) {
        ...
        const payments = await prisma.payment.findMany({
            where: { orgId, status: { not: PaymentStatus.PAID } },
            select: {
                category: true,
        ...
        payments.forEach((p) => {
            ...
            if (p.category === PaymentCategory.RECEIVABLE) {
                ...
                if (p.status === PaymentStatus.OVERDUE)
                    overdueReceivables = overdueReceivables.plus(remaining);
                if (due >= today && due <= next7Days)
                    incoming7Days = incoming7Days.plus(remaining);
            } else {
                totalPayables = totalPayables.plus(remaining);
                if (p.status === PaymentStatus.OVERDUE)
                    overduePayables = overduePayables.plus(remaining);
                if (due >= today && due <= next7Days)
                    outgoing7Days = outgoing7Days.plus(remaining);
            }
        });
    ...
    async getPartyRiskScore(partyId: string) {
        ...
        if (payments.length === 0) {
            return {
                score: 0,
                level: "LOW" as const,
                metrics: { totalPayments: 0, lateRatio: 0, avgDelayDays: 0, totalOutstanding: 0 },
            };
        }
        ...
        payments.forEach((p) => {
            ...
            if (p.status === PaymentStatus.OVERDUE) {
                ...
                const diffTime = Math.abs(
                    new Date(p.paidDate).getTime() - new Date(p.dueDate).getTime()
                );
                totalDelayDays += Math.ceil(diffTime / (1000 * 60 * 60 * 24));
            }
        });
